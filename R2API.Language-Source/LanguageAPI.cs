...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    #region New Features (Hot-Reload, Validation, Analysis)
    ...
    public static void AddOrUpdateToken(string key, string value, string language)
    {
        ...
        if (!CustomLanguage.ContainsKey(language))
            CustomLanguage[language] = new Dictionary<string, string>();

        CustomLanguage[language][key] = value;
    }
    ...
    public static void RemoveToken(string key)
    {
        ...
        if (CustomLanguage.TryGetValue(GenericLanguage, out var dict))
            dict.Remove(key);
    }
        ...
        foreach (var file in files)
        {
            ...
            foreach (var kvp in langTokens)
            {
                ...
                foreach (var token in kvp.Value)
                    Add(token.Key, token.Value, language);
            }
        }
    }
